...
class MyUtils {
  ...
  /// 跳转页面
  static Future<dynamic>? startPage(Widget page) async {
    return Get.to(page, transition: Transition.topLevel);
  }

  /// 跳转新页面，并关闭当前页面
  static Future<dynamic>? offPage(Widget page) async {
    return Get.off(page, transition: Transition.topLevel);
  }

  static Future<dynamic>? offAllPage(Widget page) async {
    return Get.offAll(page, transition: Transition.topLevel);
  }
  ...
  /// 跳转到起始页并关闭所有页面
  static Future<dynamic>? startAndRemovePage(Widget page) async {
    return Get.off(page);
  }
  ...
  static void log(dynamic object, {String? method}) {
    if (!isProd()) {
      if (!isEmpty(method)) _logger.d("方法：$method");
      _logger.d(object);
    }
  }
  ...
  static void showToast(String msg, {bool isLongTime = false}) {
    BotToast.showText(
      text: msg,
      duration: Duration(milliseconds: isLongTime ? 2800 : 1500),
    );
  }
